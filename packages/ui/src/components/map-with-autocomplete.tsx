...
const GoogleMapInternal = ({
  ...
  const [MapComponents, setMapComponents] = useState<GoogleMapsModule | null>(null);
    ...
    <APIProvider apiKey={apiKey} libraries={["places", "marker"]} language="es">
      <div style={{ height: mapHeight, width: "100%" }}>
        <Map
          ...
          mapId={mapId}
          center={center}
          onCameraChanged={(e: { detail: { center: { lat: number; lng: number } } }) =>
          ...
          onClick={(e: { detail: { latLng?: { lat: number; lng: number } | null } }) => {
...
export const MapWithAutocomplete = ({
  ...
  useEffect(() => {
    const fetchSuggestions = async () => {
      ...
      try {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const AutocompleteSuggestion = (maps.places as any).AutocompleteSuggestion;
