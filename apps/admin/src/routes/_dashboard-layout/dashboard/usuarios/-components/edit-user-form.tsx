...
export const editUserFormSchema = z
	.object({
		name: z
			...
			.trim()
			.refine(
				(val) => val.split(" ").filter((word) => word.length > 0).length >= 2,
				{
					message:
						"Debe incluir al menos nombre y apellido separados por espacios",
				},
			)
	...
	.refine(
		(data) => {
			...
			const isSimilar = nameParts.some(
				(part) => part.length > 3 && emailLocal.includes(part.slice(0, -1)),
			);
		...
		{
			message:
				"El email parece estar relacionado con el nombre. Asegúrate de que sea correcto.",
			path: ["email"],
		},
	);
				...
				<FormField
					...
					render={({ field }) => (
						<FormItem>
							...
							<FormDescription>
								Ingresa el nombre completo del usuario (mínimo nombre y
								apellido).
							</FormDescription>
							<FormMessage />
						</FormItem>
				...
				<FormField
					...
					render={({ field }) => (
						<FormItem>
							...
							<FormDescription>
								Email válido para el inicio de sesión. Debe ser de un dominio
								reconocido.
							</FormDescription>
							<FormMessage />
						</FormItem>
				...
				<FormField
					...
					render={({ field }) => (
						<FormItem>
							...
							/>
							<FormDescription>
								El rol determina los permisos y acceso del usuario en la
								plataforma.
							</FormDescription>
							<FormMessage />
						</FormItem>
