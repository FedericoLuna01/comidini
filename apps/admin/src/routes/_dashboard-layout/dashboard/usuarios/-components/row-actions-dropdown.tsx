...
export const RowActionsDropdown: React.FC<RowActionsDropdownProps> = ({
	...
	const [banDuration, setBanDuration] = useState<
		"1h" | "24h" | "7d" | "30d" | "permanent"
	>("permanent");
	const [editRole, setEditRole] = useState<"user" | "admin" | "shop">(
		user.role as "user" | "admin" | "shop",
	);
			...
			<Sheet open={showEditSheet} onOpenChange={setShowEditSheet}>
				<SheetContent>
					<SheetHeader>
						...
						<SheetDescription>
							Cambiar el rol del usuario {user.name}
						</SheetDescription>
					</SheetHeader>
			...
			<Sheet open={showBanSheet} onOpenChange={setShowBanSheet}>
				<SheetContent>
					<SheetHeader>
						...
						<SheetDescription>
							¿Estás seguro de que deseas suspender a {user.name}?
						</SheetDescription>
					</SheetHeader>
					<div className="grid gap-4 px-4">
						<div className="flex flex-col gap-4">
			...
			<AlertDialog onOpenChange={setShowDeleteConfirm} open={showDeleteConfirm}>
				<AlertDialogContent>
					<AlertDialogHeader>
						...
						<AlertDialogDescription>
							Esta acción eliminará a <b>{`${user.name} (${user.email})`}</b> y
